...
def define_model():
    ...
    # Main section
    if not st.session_state.train_tensors and st.session_state.test_tensors:
    ...
    define_model_trajectory_dimension = define_model_form.number_input(label='Trajectory Dimension (Neurons)', min_value=1, max_value=100, value=(st.session_state.sub_samples.shape)[2])
    define_model_number_classes = define_model_form.number_input(label='Number of Classes', min_value=1, max_value=100, value=len(np.unique(st.session_state.sub_labels)))
    
    define_model_submit = define_model_form.form_submit_button(label='Create Model')
    
    if define_model_submit:
        ntpn_utils.create_model(define_model_trajectory_length, define_model_number_classes, define_model_layer_width, define_model_trajectory_dimension)
    
    # Model Summary Display
    # TODO: Container to clean up the visuals
    if st.session_state.ntpn_model:
        st.session_state.ntpn_model.summary(print_fn=lambda x: st.text(x))
    else:
...
def train_model():
    ...
    with compile_tab:
        ...
        if compile_model_submit:
            ntpn_utils.compile_model(learning_rate = compile_learning_rate)
    ...
    with train_tab:
        ...
        if train_model_submit:
            ntpn_utils.train_model(train_model_epochs, view=train_model_progress)
